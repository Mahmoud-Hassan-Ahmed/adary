...
class _AllTableState extends State<AllDaysTable> {
  ...
  // Fallback flag for when the device has rotation lock.
  // OrientationBuilder handles auto-fullscreen on real rotation,
  // this flag handles forced fullscreen via the button.
  ...
  void _onPageChanged(int idx, ClassRoomController ctrl) {
    setState(() => _currentPage = idx);
    ctrl.onPageChanged(idx);
  }
  ...
  // ── Full-screen view ─────────────────────────────────────────────────────
  // FittedBox is INSIDE each PageView item so vertical swipes correctly
  // navigate between tables rather than being intercepted by a wrapper.
  Widget _buildFullScreenView(BuildContext context) {
    ...
    if (classesCount == 0 || daysCount == 0) {
      return Stack(children: [
        ...
        _overlayBar(context, topPad, null, null),
      ]);
    }
    ...
    // renderH targets cellH = 100 in bounded LayoutBuilder.
    final double renderH = (daysCount + 1) * 100.0;
    final double virtualH = renderH * 1.2;

    return Stack(children: [
      // ── PageView — swipe up/down to switch classrooms ──────────────
      GetBuilder<ClassRoomController>(builder: (ctrl) {
        _syncPageIfNeeded(ctrl.tablesList.length);
        return PageView.builder(
          controller: _pageCtrl,
          scrollDirection: Axis.vertical,
          physics: const BouncingScrollPhysics(),
          itemCount: ctrl.tablesList.length,
          onPageChanged: (idx) => _onPageChanged(idx, ctrl),
      ...
      // ── Overlay bar (exit + class badge + time toggle) ─────────────
      GetBuilder<ClassRoomController>(builder: (ctrl) {
        final idx =
            _currentPage.clamp(0, (ctrl.tablesList.length - 1).clamp(0, 9999));
        final name =
            ctrl.tablesList.isEmpty ? '' : ctrl.tablesList[idx]['classroom_name']?.toString() ?? '';
        final pageLabel =
            ctrl.tablesList.isEmpty ? '' : '${idx + 1} / ${ctrl.tablesList.length}';
        return _overlayBar(context, topPad, name, pageLabel);
      }),
    ]);
  }

  Widget _overlayBar(BuildContext context, double topPad, String? className,
      String? pageLabel) {
    return Positioned(
      top: topPad + 6,
      left: 8,
  ...
  Widget _buildNormalView(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        // ③ Class header — reactive, pinned
        GetBuilder<ClassRoomController>(builder: (ctrl) {
          if (ctrl.tablesList.isEmpty) return const SizedBox.shrink();
          final idx = _currentPage.clamp(0, ctrl.tablesList.length - 1);
          return _ClassHeader(
            className:
                ctrl.tablesList[idx]['classroom_name']?.toString() ?? '',
            currentIndex: idx,
            total: ctrl.tablesList.length,
          );
        }),
        ...
        Expanded(
          child: GetBuilder<ClassRoomController>(
            builder: (classRoomController) {
              _syncPageIfNeeded(classRoomController.tablesList.length);

              if (classRoomController.isLoadingTables) {
                return const Center(
              ...
              if (classRoomController.tablesList.isEmpty) {
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'لا يوجد جدول',
                        style: AlMaraiaBold.copyWith(
                            fontSize: 16, color: AppColors.GREYFONTCOLOR),
                      ),
                    ],
                  ),
                );
              ...
              return PageView.builder(
                ...
                onPageChanged: (idx) =>
                    _onPageChanged(idx, classRoomController),
                itemBuilder: (context, index) {
                  return AppTable(
                    table: classRoomController.tablesList[index],
                    isInstructor: false,
                    showTime: _showTime,
                  );
                },
        ...
        // ⑤ Page indicator
        _PageDots(currentPage: _currentPage, total: _prevTableCount),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    // OrientationBuilder auto-detects landscape rotation — device rotation
    // itself triggers fullscreen without any button press.
    return OrientationBuilder(builder: (context, orientation) {
      ...
      return Scaffold(
        ...
        // AppBar is hidden in full-screen mode — table occupies the full screen.
