...
class ExaminationState extends State<Examination> {
  ...
  final getExames = sl<GetExamsUseCase>();
  @override
  void initState() {
  ...
  Future<void> _fetchPage(int pageKey) async {
    var result = await getExames(entity..page = pageKey);
    try {
    ...
    return BlocProvider(
      ...
      child: BlocBuilder<ExamBloc, ExamState>(
        builder: (context, state) {
          ...
          return SafeArea(
            child: Scaffold(
              appBar: MyAppBar(title: 'لجان الاختبارات ', actions: [
                if (AppUtils.checkPermission([
                  '/exams_management/edit-exam-hall-group/',
                ]))
                  IconButton(
                    onPressed: () {
              ...
              //   itemCount: 5,
              //   itemBuilder: (context, index) => const ExamCard(),
              // ),
              body: PagedListView<int, Exam>(
